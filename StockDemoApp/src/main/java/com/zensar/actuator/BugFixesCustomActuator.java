...
public class BugFixesCustomActuator {
	...
	@WriteOperation
	public boolean addBugFixes(@Selector String version, String bugFixes) {
	bugFixesByVersionMap.put(version, Arrays.asList(bugFixes.split(",")));
     return true;
	}
	
	@DeleteOperation
	public boolean deleteBugFixes(@Selector String version) {
	 if(bugFixesByVersionMap.remove(version) != null)
	 {
		 return  true;
	 }
	 return false;
